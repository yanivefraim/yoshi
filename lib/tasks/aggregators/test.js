...
module.exports = (gulp, plugins, options) => {
  ...
  gulp.task('test:duringwatch', cb => runSequence('mocha', utils.handleRunSequenceErrors(cb)));
  gulp.task('test', cb => runSequence('mocha', 'protractor', utils.handleRunSequenceErrors(cb)));

  // gulp.task('test:jasmine', cb => runSequence('jasmine', utils.handleRunSequenceErrors(cb)));
  // gulp.task('test:mocha', cb => runSequence('mocha', utils.handleRunSequenceErrors(cb)));
  // gulp.task('test:e2e', cb => runSequence('protractor', utils.handleRunSequenceErrors(cb)));

  // gulp.task('test:jasmine', cb =>
  //   runSequence('test:unit:jasmine', 'test:e2e', utils.handleRunSequenceErrors(cb)));
  // gulp.task('test:mocha', cb =>
  //   runSequence('test:unit:mocha', 'test:e2e', utils.handleRunSequenceErrors(cb)));
};
