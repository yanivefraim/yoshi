...
module.exports = (gulp, plugins, {watch}) => {
  ...
  gulp.task('_mocha', () => {
    ...
    const mochSetupPath = path.resolve(path.join('test', 'mocha-setup.js'));
    ...
    return gulp.src(mochaGlobs, {read: false})
      .pipe(plugins.spawnMocha({
        ...
        require: [
          ...
          utils.isTypescriptProject() ? 'ts-node/register' : 'babel-register',
          utils.exists(mochSetupPath) ? mochSetupPath : ''
        ]
      }))
